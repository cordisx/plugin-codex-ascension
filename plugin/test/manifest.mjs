...
test('exports a capability-free structured CordisX plugin', () => {
  ...
  assert.deepEqual(backdrop.stages.map(stage => stage.ambience), [
    'dormant',
    'ember',
    'forged',
    'luminous',
  ...
  assert.ok(
    backdrop.stages.every(stage => stage.portrait.mediaType === 'image/png' && stage.portrait.data.length > 100_000),
  );
...
test('registers an explicit reasoning replacement and a legacy composed backdrop', () => {
  ...
  Object.defineProperty(hostHandle, 'control', {
    get() {
      throw new Error('plugin must not read or forge the Host-projected control lease');
    },
  });
  apply({
